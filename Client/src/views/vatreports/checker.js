...
const VatReport = (props) => {
	...
	const getPeriods = (userId) => {
		...
		setShowLoader(true);
		callApi("get", ApiConstants.vatreports.vatreportperiodsforothers, {user_id: userId}, true)
			.then((response) => {
				setShowLoader(false);
				if (response && response.status_code === 200) {
		...
		<React.Fragment>
										...
										<Card className="rounded">
											<Card.Body className="px-4 pt-4 pb-0">
												<div className="report-tile">
													...
													<div className="report-name mt-3">{getReportName(item)}</div>
												</div>
			...
			{reportsData && (
				<div
					...
					<PDFExport ref={pdfExportComponent} paperSize="A4" margin={30} fileName={getReportName(selectedReport)} author="VatzApp">
					...
					<div className="px-4 py-5">
						<Row>
							...
							<Col className="col-12 my-2 d-flex justify-content-center">
								<Button variant="primary" onClick={() => onEntrySubmit("Pending", reportsData)}>
									Request for Approval
								</Button>
							</Col>
						</Row>
